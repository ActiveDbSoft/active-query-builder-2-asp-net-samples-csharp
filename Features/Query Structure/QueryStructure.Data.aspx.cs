...
namespace Samples
{
    public partial class QueryStructure
    {
        ...
        private void DumpDataSourceInfo(StringBuilder stringBuilder, DataSource dataSource)
        {
            ...
            {
                stringBuilder.AppendLine("&nbsp;&nbsp;subquery sql: " + ((DataSourceQuery)dataSource).Query.GetResultSQL() + "<br />");
            }
        ...
        private void DumpExpression(StringBuilder stringBuilder, string indent, SQLExpressionItem expression)
        {
            ...
            else
            {
				...
				string s = expression.GetSQL(expression.SQLContext.SQLBuilderExpression);
                stringBuilder.AppendLine(indent + s + "<br />");
            }
        }
        ...
        {
            for (int i = 0; i < queryBuilder.GetSubQueryList().Count; i++)
            {
                if (stringBuilder.Length > 0)
                {
                    stringBuilder.AppendLine("<br />");
